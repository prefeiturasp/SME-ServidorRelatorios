...
namespace SME.SR.Data
{
    public class PlanoAeeVersaoRepository : IPlanoAeeVersaoRepository
    {
        ...
        public async Task<IEnumerable<PlanosAeeDto>> ObterPlanoAEE(FiltroRelatorioPlanosAeeDto filtro)
        {
	        ...
	        var saoTurmasEspecificas = !filtro.CodigosTurma.EstaFiltrandoTodas();

	        if (saoTurmasEspecificas)
	        {
		        query.AppendLine(@"with planosAtuais as
					(
	        ...
	        query.AppendLine(@"	select pa.id, d.dre_id dreId, d.nome as dreNome,
			        d.abreviacao as dreAbreviacao,
			        u.ue_id as ueCodigo,
			        u.nome as ueNome,
			        u.tipo_escola as tipoEscola,
	        ...
	        query.AppendLine(saoTurmasEspecificas
	        ...
	        query.AppendLine(@"	inner join turma t on t.id = pa.turma_id
					inner join ue u on u.id = t.ue_id 
					inner join dre d on d.id = u.dre_id
					left join usuario u2 on u2.id = pa.responsavel_paai_id
					left join usuario u3 on u3.id = pa.responsavel_id
