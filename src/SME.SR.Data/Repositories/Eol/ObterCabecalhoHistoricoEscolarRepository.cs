...
namespace SME.SR.Data
{
    public class ObterCabecalhoHistoricoEscolarRepository : IObterCabecalhoHistoricoEscolarRepository
    {
        ...
        public async Task<CabecalhoDto> ObterCabecalhoHistoricoEscolar(string codigoDre)
        {
            ...
            {
                conn.Open();
                var cabecalho = await conn.QueryFirstOrDefaultAsync<CabecalhoDto>(query, new { codigoDre });
                conn.Close();
                return cabecalho;
            }
        }
