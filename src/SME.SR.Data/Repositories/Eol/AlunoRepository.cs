...
namespace SME.SR.Data
{
    public class AlunoRepository : IAlunoRepository
    {
		...
		public async Task<int> ObterTotalAlunosPorTurmasDataSituacaoMatriculaAsync(string anoTurma, string ueCodigo, int anoLetivo, long dreCodigo, DateTime dataReferencia, int[] modalidades)
        {
			...
			query.AppendLine("	SELECT DISTINCT mte.cd_turma_escola,");
			query.AppendLine("					m.cd_aluno");
			query.AppendLine("	FROM v_matricula_cotic m");
			query.AppendLine("		INNER JOIN matricula_turma_escola mte");
			query.AppendLine("			ON m.cd_matricula = mte.cd_matricula");
			query.AppendLine("		INNER JOIN turma_escola te");
			query.AppendLine("			ON mte.cd_turma_escola = te.cd_turma_escola");
			query.AppendLine("		INNER JOIN serie_turma_escola ste");
			query.AppendLine("			ON te.cd_turma_escola = ste.cd_turma_escola");
			query.AppendLine("		INNER JOIN serie_turma_grade stg");
			query.AppendLine("			ON ste.cd_serie_ensino = stg.cd_serie_ensino");
			query.AppendLine("		INNER JOIN serie_ensino se");
			query.AppendLine("			ON stg.cd_serie_ensino = se.cd_serie_ensino");
			query.AppendLine("		INNER JOIN etapa_ensino ee");
			query.AppendLine("			ON se.cd_etapa_ensino = ee.cd_etapa_ensino");
			query.AppendLine("		INNER JOIN v_cadastro_unidade_educacao ue");
			query.AppendLine("			ON te.cd_escola = ue.cd_unidade_educacao");
			query.AppendLine("	WHERE te.an_letivo = @anoLetivo AND");
			query.AppendLine("		  te.cd_tipo_turma = 1 AND");
			query.AppendLine("		  mte.cd_situacao_aluno in (1, 6, 10, 13, 5) AND");
			query.AppendLine("		  mte.dt_situacao_aluno < @dataFim AND");
			query.AppendLine("		  se.sg_resumida_serie = @anoTurma AND");
			query.AppendLine("		  ee.cd_etapa_ensino in (@modalidades)");
			query.AppendLine($"		  {(dreCodigo > 0 ? " AND ue.cd_unidade_administrativa_referencia = @dreCodigo" : string.Empty)}");
			query.AppendLine($"		  {(!string.IsNullOrWhiteSpace(ueCodigo) ? " AND ue.cd_unidade_educacao = @ueCodigo" : string.Empty)}");
			query.AppendLine("UNION");
			query.AppendLine("SELECT DISTINCT mte.cd_turma_escola,");
			query.AppendLine("					m.cd_aluno");
			query.AppendLine("	FROM v_historico_matricula_cotic m");
			query.AppendLine("		INNER JOIN historico_matricula_turma_escola mte");
			query.AppendLine("			ON m.cd_matricula = mte.cd_matricula");
			query.AppendLine("		INNER JOIN turma_escola te");
			query.AppendLine("			ON mte.cd_turma_escola = te.cd_turma_escola");
			query.AppendLine("		INNER JOIN serie_turma_escola ste");
			...
			query.AppendLine("	WHERE te.an_letivo = @anoLetivo AND");
			query.AppendLine("		  te.cd_tipo_turma = 1 AND");
			query.AppendLine("		  mte.cd_situacao_aluno in (5, 10) AND");
			query.AppendLine("		  se.sg_resumida_serie = @anoTurma AND");
			query.AppendLine("		  ee.cd_etapa_ensino in (@modalidades)");
			query.AppendLine($"		  {(dreCodigo >  0 ? " AND ue.cd_unidade_administrativa_referencia = @dreCodigo" : string.Empty)}");
			query.AppendLine($"		  {(!string.IsNullOrWhiteSpace(ueCodigo) ? " AND ue.cd_unidade_educacao = @ueCodigo" : string.Empty)})");
			query.AppendLine("SELECT COUNT(DISTINCT cd_aluno)");
