...
namespace SME.SR.Data.Repositories.ElasticSearch.Base
{
    public abstract class RepositorioElasticBase<T> : IRepositorioElasticBase<T> where T : class
    {
        ...
        public async Task<long> ObterTotalDeRegistroAPartirDeUmaCondicaoAsync<H>(string indice, string nomeConsulta, Func<QueryContainerDescriptor<H>, QueryContainer> request, object parametro = null)
        where H : class
        {
            try
            {
                ISearchResponse<T> response = await servicoTelemetria.RegistrarComRetornoAsync<ISearchResponse<H>>(async () =>
                                                                               await _elasticClient.SearchAsync<H>(s => s.Index(indice)
                                                                                                                         .Query(request)
                                                                                                                         .Scroll(TEMPO_CURSOR)
                                                                                                                         .Size(QUANTIDADE_RETORNO)),
