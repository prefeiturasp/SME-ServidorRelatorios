            ...
            if (boletim.ComponentesCurriculares != null && boletim.ComponentesCurriculares.Any())
            {
                foreach (var componenteCurricular in boletim.ComponentesCurriculares)
                {
                    ...
                    if (componenteCurricular.Nota)
                    {
                        ...
                        componenteCurricular.NotaFinal = !string.IsNullOrEmpty(frequenciaFinal) && frequenciaFinal.ToCharArray().Where(c => c != ',').All(f => char.IsDigit(f)) ? ObterSintese(double.Parse(frequenciaFinal ?? "0"), mediasFrequencia, false, false) : frequenciaFinal;
