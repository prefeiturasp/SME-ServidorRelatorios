...
namespace SME.SR.Application
{
    public class ObterTipoNotaQueryHandler : IRequestHandler<ObterTipoNotaQuery, string>
    {
        ...
        public async Task<string> Handle(ObterTipoNotaQuery request, CancellationToken cancellationToken)
        {
            ...
            PeriodoFechamentoBimestre periodoFechamentoBimestre = await _periodoFechamentoRepository.ObterPeriodoFechamentoTurmaAsync(request.Turma.DreUe.UeId, request.Turma.DreUe.DreId, request.Turma.AnoLetivo, bimestreFechamento, request.PeriodoEscolar?.Id);
