        ...
        public async Task<Unit> Handle(GerarRelatorioAtaFinalExcelCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                for (int i = 0; i < dadosAgrupadosTurma.Count(); i++)
                {
                    ...
                    await mediator.Send(new InserirFilaRabbitCommand(new PublicaFilaDto(mensagem, RotasRabbit.RotaRelatorioCorrelacaoInserir, ExchangeRabbit.ExchangeSgp, codigoCorrelacao, request.UsuarioRf)));
                ...
                foreach (var codigoCorrelacao in lstCodigosCorrelacao)
                    servicoFila.PublicaFila(new PublicaFilaDto(ObterNotificacao(modalidade, codigoCorrelacao.Value), RotasRabbit.RotaRelatoriosProntosSgp, ExchangeRabbit.ExchangeSgp, codigoCorrelacao.Key));
