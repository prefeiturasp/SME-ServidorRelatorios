    ...
    public class RelatorioSondagemMatConsolidadoAdtMultiUseCase : IRelatorioSondagemMatConsolidadoAdtMultiUseCase
    {
        ...
        public async Task<string> Executar(FiltroRelatorioSincronoDto request)
        {
            ...
            if (filtros.AnoLetivo < 2022 && filtros.Semestre > 0)
                dataReferencia = (await mediator.Send(new ObterDataPeriodoFimSondagemPorSemestreAnoLetivoQuery(filtros.Semestre, filtros.AnoLetivo)))?.PeriodoFim ?? DateTime.Now;
            else if (filtros.AnoLetivo >= 2022 && filtros.Bimestre > 0)
                dataReferencia = (await mediator.Send(new ObterDataPeriodoFimSondagemPorBimestreAnoLetivoQuery(filtros.Bimestre, filtros.AnoLetivo)))?.PeriodoFim ?? DateTime.Now;
